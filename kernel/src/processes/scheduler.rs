...
use crate::{
    autogenerated::userspace_programs::{IDLE, INIT, PROGRAMS},
    cpu, debug, info,
    klibc::elf::ElfFile,
    memory::page_tables,
...
fn prepare_next_process() {
    ...
    let next_process_ref =
        process_list::next_runnable().expect("There must always be the IDLE process");

    // If the idle process is the last process we can shut down the system
